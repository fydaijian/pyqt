...
class WinForm(QWidget):
    ...
    def myFunction(self):
        ...
        # self.timer.stop()
        time = QDateTime.currentDateTime()
        first = time.toString("yyyy-mm-dd hh:mm:ss")

        self.label.setText('fist-----' + first)

        # time.sleep(5)

        for i in range(1000000000):  # 此代码远远超过 5 秒
            if i % 100 == 0:
                print(i)
        # str = time.toString("yyyy-mm-dd hh:mm:ss")
        #
        # time = QDateTime.currentDateTime()
        # next= time.toString("yyyy-mm-dd hh:mm:ss")
        #
        # self.label.setText('next-----' + next)
        # self.label.setText('next-----' + str)
